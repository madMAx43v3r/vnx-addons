...
public:
	...
	uint64_t read_file(const std::string& path, 
			const std::function<void(::vnx::Buffer)>& _callback = std::function<void(::vnx::Buffer)>(),
			const std::function<void(const std::exception&)>& _error_callback = std::function<void(const std::exception&)>());
...
private:
	...
	std::map<uint64_t, std::pair<std::function<void(::vnx::Buffer)>, std::function<void(const std::exception&)>>> vnx_queue_read_file;
